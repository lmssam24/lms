...
const CourseGrid = () => {
  ...
  useEffect(() => {
    ...
    FacultyService.getCourseCateogery().then((res) => {
      if (res && res.data && res.data.data) {
        ...
        StudentService.listCourseDetails().then((res) => {
          if (res.status === 200) {
              ...
              <div key={"manage_course_grid" + index} className="col-md-6">
                <div className="coach-item wow fadeInUp delay-0-4s">
                  <div className="coach-image">
                    <img src={getImgSrc(course?.image_1)} alt="Coach" />
                  </div>
                  <div className="coach-content">
                    {/* <span className="label">Coming Soon</span> */}
                    <h4>
                      <Link href={redirectToTemplate(course.id)}>{course.title}</Link>
                    </h4>
                    ...
                    {showEnroll && (
                      <div className="flex-Just-Center mb-10">
                        ...
                        <Button style={{ width: "70%" }} onClick={() => (window.location.href = redirectToTemplate(course.id))} variant="primary">
                          View Course Details
                        </Button>
                      </div>
                    )}
  ...
  return (
    <Layout>
      ...
      <section className="course-left-area py-100 rpy-100">
                  ...
                  <div className="widget widget-menu wow fadeInUp delay-0-4s">
                    ...
                    <ul>
                      ...
                      {courseCateogeries.length > 0 &&
                        courseCateogeries.map((element, index) => {
                          return (
