...
function AddAssignment() {
  ...
  const submitAssignment = (e) => {
    ...
    if (isDeleteOpen && isDelete) {
      FacultyService.deleteAssignment(id)
        .then((res) => {
          if (res.status == 200) {
            setError("");
    ...
    if (validation_ == false) {
      if (!isEditOpen && !isDeleteOpen) {
        FacultyService.addAssignment(moule_data)
          .then((res) => {
            if (res.status == 200) {
              setError("");
              setInput({ ...initialState });
              ...
              if (assignment_material?.type !== undefined) {
                FacultyService.uploadAssignemnt(file_data)
                  .then((res) => {
        ...
        FacultyService.updateAssignment(moule_data, id)
          .then((res) => {
    ...
    assignmentList.map((element, idx) => {
      (action = (
        <>
          ...
          <button
            onClick={() => {
              handleDeleteAssignment(element);
            }}
            className="btn btn-default btn-sm float-right"
          >
                    ...
                    <form onSubmit={submitAssignment} autoComplete="off">
                      <div className="col-md-12">
                        <div className="form-group row">
                          ...
                          <div className="col-sm-4 col-md-4">
                            <input id="quiz_title" name="assignment_title" value={input.assignment_title} onChange={handleChange} type="text" className="form-control" placeholder="Title" />
                          </div>
                        </div>
                        <div className="form-group row">
                          <label className="col-sm-3 col-form-label form-label">Select Module</label>
                          <div className="col-sm-4">
                            <select id="quizId" defaultValue="" name="module_id" value={input.module_id} onChange={handleChange} className="form-select">
                      ...
                      <div className="col-md-12">
                        <div className="form-group ">
                          ...
                          <textarea className="form-control" id="question" name="question" value={input.question} onChange={handleChange} rows="3"></textarea>
                        </div>
                      </div>
                      <div className="col-md-12">
                        <div className="form-group ">
  ...
  function ShowList() {
    return (
      <div className="content">
        {assignmentList.length === 0 && <label className="col-form-label form-label"> Looks like Assignment list is empty ; start adding assignments</label>}
        <hr />
        {assignment_list_.length > 0 && <PaginationTable data={assignment_list_} columns={columns} />}
      </div>
    );
  }
}
