...
class AddQuiz(APIView):
    ...
    def get(self, request):
        data = Quiz.objects.filter(teacher=request.user.id)
...
class AddQuizQuestions(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():  
            try:  
                quiz_obj = Quiz.objects.get(id=serializer.validated_data['quiz_id'], teacher=user.id)
            except:
    ...
    #     users_quiz = Quiz      
    #     data = QuizQuestions()
    #     serializer = QuizQuestionsSerializer_(data)
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            #     return Response({"message":"Assignment already added for the current student"},
                # status=200)
            # local_save_file_path = os.path.join(settings.BASE_DIR,'media', str(request.user))   
            # file_path = f"{str(request.user)}/{file.name}" 
            # file_save_path_for_s3 = f"Assignments/{file_path}"
            # s3_file_link = settings.S3_LINK + file_save_path_for_s3
            # assignment = Assignment(
            #     teacher_id=user.id,
            #     file_upload_url=s3_file_link,
            #     title=serializer.validated_data['title'],
            #     question=serializer.validated_data['question']
            #     )
            # assignment.save()
        
            # fs = FileSystemStorage(location=local_save_file_path)
            # filename = fs.save(file.name, file) 
            # fileurl = fs.url(filename)           
            
            
            # uploaded = upload_to_aws(local_save_file_path+fileurl, settings.BUCKET_NAME, file_save_path_for_s3)
            
            # uploaded = upload_to_aws(file, settings.BUCKET_NAME, file_save_path)
...
class CreateMeeting(APIView):
    ...
    def get(self, request):
        data = TeacherMeeting.objects.filter(teacher=request.user.id).last()
